"""Column-structure parity check between scan-inferred schema and live TiDB."""
...
def parse_ddl_columns(create_table_sql: str) -> dict[str, str]:
    """Parse a CREATE TABLE into {column_name: column_type}."""
    cols: dict[str, str] = {}
    ...
    for line in body.split(","):
        ...
        if line.upper().startswith(
            ("PRIMARY KEY", "KEY", "UNIQUE", "FOREIGN", "INDEX", "CONSTRAINT")
        ):
            continue
...
def compare_structure(
    cfg: TiShiftConfig, *, collection: str, expected_ddl: str
) -> StructureDiff:
    """Compare convert-time DDL against live TiDB DDL."""
